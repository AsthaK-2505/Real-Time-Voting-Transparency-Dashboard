 ...
 */
const DistrictCard: React.FC<DistrictCardProps> = memo(
  ({ district, anomalyScore, candidates }) => {
    const turnoutRate = (
      (district.votes / district.registeredVoters) *
      100
    ).toFixed(1);
    ...
    return (
      <div
        ...
        role="article"
        aria-label={`${district.name} statistics`}
      >
        <div className="district-header">
          <div>
            ...
            <p className="district-meta">
              <span
                aria-label={`${district.votes} votes cast out of ${district.registeredVoters} registered voters`}
              >
                {district.votes.toLocaleString()} /{" "}
                {district.registeredVoters.toLocaleString()} votes
              </span>
            </p>
          </div>
          <div
        ...
        <div className="district-stats">
          ...
          <div className="stat-item">
            ...
            <span
              className="stat-value"
              aria-label={`District is ${district.status}`}
            >
        ...
        <div className="vote-distribution">
          ...
          {candidates.map((candidate) => {
            ...
            const percentage =
              district.votes > 0
                ? ((votes / district.votes) * 100).toFixed(1)
                : "0";
