...
import { District, Candidate, VoteHistoryEntry, CandidateVotes } from '../types';
 ...
 */
export const VoteTrendsChart: React.FC<VoteTrendsChartProps> = memo(({ voteHistory, candidates }) => {
  ...
  const data = {
    ...
    datasets: candidates.map(candidate => ({
      ...
      data: voteHistory.map(entry => entry.candidateTotals[candidate.id as keyof CandidateVotes]),
  ...
  return (
    <div style={{ height: '300px' }} role="img" aria-label="Vote trends over time chart">
 ...
 */
export const DistrictVotesChart: React.FC<DistrictVotesChartProps> = memo(({ districts, candidates }) => {
  const data = {
    labels: districts.map(d => d.name.split(' - ')[1] || d.name),
    datasets: candidates.map(candidate => ({
      label: candidate.name,
      data: districts.map(d => d.candidateVotes[candidate.id as keyof CandidateVotes]),
  ...
  return (
    <div style={{ height: '300px' }} role="img" aria-label="Votes by district chart">
 ...
 */
export const VoteShareChart: React.FC<VoteShareChartProps> = memo(({ candidateTotals, candidates }) => {
  const data = {
    labels: candidates.map(c => c.name),
    datasets: [
      {
        data: candidates.map(c => candidateTotals[c.id as keyof CandidateVotes] || 0),
      ...
      legend: {
        ...
        labels: {
          ...
          generateLabels: chart => {
            ...
            const total = (chartData.datasets[0].data as number[]).reduce((sum, val) => sum + val, 0);
  ...
  return (
    <div style={{ height: '300px' }} role="img" aria-label="Overall vote share chart">
 ...
 */
export const TurnoutChart: React.FC<TurnoutChartProps> = memo(({ districts }) => {
  ...
  const data = {
    ...
    datasets: [
      {
        label: 'Turnout Rate (%)',
        data: sortedDistricts.map(d => Number(((d.votes / d.registeredVoters) * 100).toFixed(1))),
  ...
  return (
    <div style={{ height: '350px' }} role="img" aria-label="Turnout rate by district chart">
      <Bar data={data} options={options} />
    </div>
  );
});
