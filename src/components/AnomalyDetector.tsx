 ...
 */
const AnomalyDetector: React.FC<AnomalyDetectorProps> = memo(
  ({ anomalies }) => {
    if (!anomalies || anomalies.length === 0) {
      return (
        <div
          className="anomaly-detector"
          role="region"
          aria-label="Anomaly detection status"
        >
      ...
      >
        <div className="anomaly-header">
          ...
          <div className="anomaly-summary" role="status">
            {highSeverity > 0 && (
              <span
                className="severity-count high"
                aria-label={`${highSeverity} high severity anomalies`}
              >
                {highSeverity} High
              </span>
            )}
            {mediumSeverity > 0 && (
              <span
                className="severity-count medium"
                aria-label={`${mediumSeverity} medium severity anomalies`}
              >
        ...
        <div className="anomaly-list" role="list">
          {anomalies.slice(0, 10).map((anomaly, index) => (
            <div
              ...
              role="listitem"
              aria-label={`${anomaly.severity} severity anomaly in ${
              ...
              <div className="anomaly-content">
                <div className="anomaly-title">
                  ...
                  <span
                    ...
                    style={{
                      backgroundColor:
                        getSeverityColor(anomaly.severity) + "20",
                    }}
                  >
                    {anomaly.type}
                  </span>
                ...
                {anomaly.voteRate && (
                  <div className="anomaly-details">
                    ...
                    <span>
                      Expected: {anomaly.expectedRate?.toFixed(0)} votes/min
                    </span>
                  </div>
                )}
              </div>
